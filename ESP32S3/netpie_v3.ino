...
void sendDataToNetpie() {
  ...
  doc["data"]["isTooDark"] = isTooDark;
  doc["data"]["isBadAir"] = isBadAir;
  doc["data"]["feelsLike"] = feelsLike;
...
void loop() {
  ...
  if (Serial1.available()) {
    ...
    int t, h, l, d, b, f;
    if (sscanf(line.c_str(), "T:%d,H:%d,L:%d,D:%d,B:%d,F:%d", &t, &h, &l, &d, &b, &f) == 6) {
      temp = t;
      humi = h;
      light = l;
      isTooDark = d;
